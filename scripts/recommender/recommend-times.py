...
# Return a set of optimal times (pareto-optimal for minimal attendance and preferred times) for each day of the week for both forecasts
def paretoset_optimize_days_and_hours(current_week_forecast, next_week_forecast, user_exercise_categories, user_activities):
    ...
    def pareto_optimize_week(df):
        ...
        for day_df in day_dfs:
            recommended_week_dfs.append(pareto_optimize_day(day_df))
    ...
    current_week_activity = pareto_optimize_by_activity(current_week_forecast, user_activities)
    ...
    # Next week: pareto-optimize by exercise category and activity
    next_week_exercise_category = pareto_optimize_by_exercise_category(next_week_forecast, user_exercise_categories)
    next_week_activity = pareto_optimize_by_activity(next_week_forecast, user_activities)
